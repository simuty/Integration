...
export default class RedPacketController extends Controller {
  // 生成红包数据
  public async create() {
    const result = await this.service.redPacket.createRp(this.ctx.request.body);
  ...
  public async grap_lua() {
    const { userId, redpacketId } = this.ctx.request.body;
    const result = await this.service.redPacket.getRedPacket_redis_lua(redpacketId, userId);
    this.ctx.body = result;
  }
}
