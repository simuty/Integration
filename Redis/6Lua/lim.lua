...
local function contains(source_str, sub_str)
    ...
    if start_pos == nil then return false end
...
local function acquire(key, permits, curr_mill_second, context)
    ...
    if (type(last_mill_second) ~= 'boolean' and last_mill_second ~= nil) then
        local reverse_permits = math.floor(
                                    ((curr_mill_second - last_mill_second) /
                                        1000) * rate)
...
local function init(key, max_permits, rate, apps)
    ...
    if (org_max_permits == nil) or
        (apps ~= org_apps or rate ~= org_rate or max_permits ~= org_max_permits) then
        redis.pcall("HMSET", key, "max_permits", max_permits, "rate", rate,
                    "curr_permits", max_permits, "apps", apps)
    end
    return 1
end
